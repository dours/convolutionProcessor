...
vector<Instruction> computePseudoBlur;
...
void runProgram(ProcessorState& ps) { 
  initInitCommon();
  initComputePseudoBlur();
  for (int i = 0; i < initCommon.size(); ++i) initCommon.at(i).execute(ps);
  while (1) for (int i = 0; i < computePseudoBlur.size(); ++i) computePseudoBlur.at(i).execute(ps);
}
